...
function repeatString(str, times) {
  return str.repeat(Math.sign(times) === 1 ? times : 0);
}
...
function removeLastOccurrences(str, value) {
  const valueLastIndex = str.lastIndexOf(value);
  return str.includes(value)
    ? `${str.slice(0, valueLastIndex)}${str.slice(valueLastIndex + value.length)}`
    : str;
}
...
function countVowels(str) {
  ...
  return str
    ...
    .reduce(
      (vowelSum, letter) =>
        vowelSum + (vowels.includes(letter.toLowerCase()) ? 1 : 0),
      0
    );
}
...
function reverseWords(str) {
  const reversedStr = str.split(' ').map((word) => {
    let reversed = '';
...
function invertCase(str) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    if (strArr[i].match(/[A-Z]/)) {
      ...
      newStrArr.push(strArr[i].toUpperCase());
    }
  }
